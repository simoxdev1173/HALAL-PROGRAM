...
const CompanySearch = () => {
  ...
  return (
    <section className="relative py-16 lg:py-24 xl:py-32 bg-[#F8FAFC] overflow-hidden border-y border-stone-200 shadow-[var(--shadow-ind-card)]" dir={isRtl ? "rtl" : "ltr"}>
          ...
          >
            ...
            <div className="bg-white rounded-2xl lg:rounded-3xl p-5 lg:p-6 relative shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100">
              ...
              <div className="flex bg-slate-100/70 p-1 rounded-xl lg:rounded-2xl mb-6 relative z-0 border border-slate-100 w-full">
                {([
                  { id: "license", label: t("search.tabs.license"), icon: FileText },
                  { id: "company", label: t("search.tabs.company"), icon: Building }
                ] as const).map((tab) => {
              ...
              >
                ...
                <button
                  ...
                  className={`absolute ${isRtl ? "left-1 lg:left-1.5" : "right-1 lg:right-1.5"} w-10 lg:w-12 h-10 lg:h-12 bg-[#007A55] text-white rounded-lg lg:rounded-[14px] flex items-center justify-center hover:bg-[#006344] active:translate-y-[2px] disabled:opacity-40 transition-all shadow-md shadow-[#007A55]/20 group cursor-pointer`}
                >
                  {isSearching ? (
                    <div className="w-4 lg:w-5 h-4 lg:h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  ) : (
                    <ArrowLeft size={18} strokeWidth={2.5} className={`lg:w-5 lg:h-5 transition-transform ${isRtl ? "group-hover:-translate-x-1" : "rotate-180 group-hover:translate-x-1"}`} />
                  )}
                </button>
              </form>
            </div>
