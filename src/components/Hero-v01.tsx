          ...
          {CTA_DATA.map((card, index) => (
            <motion.div
              ...
              className={`relative flex flex-col p-8 rounded-3xl border transition-all duration-300 ${
                ...
                : "bg-slate-50 border-slate-100 hover:border-emerald-200 hover:shadow-xl hover:shadow-emerald-900/5"
              }`}
            >
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${
                card.secondary ? "bg-emerald-500/20 text-emerald-400" : "bg-emerald-100 text-emerald-700"
              }`}>
                <card.icon size={24} />
              </div>

              <span className={`text-xs font-bold uppercase tracking-wider mb-2 ${
                card.secondary ? "text-emerald-400" : "text-emerald-600"
              }`}>
                {card.tag}
              </span>
              ...
              <button className={`flex items-center gap-2 font-bold text-sm group ${
                card.secondary ? "text-white" : "text-slate-900"
              }`}>
                {card.linkText}
